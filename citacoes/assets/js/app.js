...
/* select elements */
const elementQuoteBtn = document.querySelector('#quote_btn');
const elementQuoteText = document.querySelector('.text');
const elementQuoteAuthor = document.querySelector('.author');

function getQuote() { 
    const index = Math.floor(Math.random() * quotes.length);
    
    elementQuoteText.textContent = quotes[index].quote;
    elementQuoteAuthor.textContent = quotes[index].author;
};

elementQuoteBtn.addEventListener('click', getQuote);